...
class LoaderConfiguration {
    ...
    fun loadNotificationKafkaSender(
        ...
        val kafkaSenders = loaders.map { it.type }.associateWith { type ->
            createProducerForNotifications(loadProperties.brokerReplicaSet, type)
        }
    ...
    ): ConsumerWorkerHolder<LoadTask> = ConsumerWorkerHolder(
    ...
    private fun loadWorkers(
        ...
        loadProperties: LoadProperties,
        loadRunnerParalleller: LoadRunnerParalleller
    ): List<ConsumerBatchWorker<LoadTask>> {
        return (0 until loadProperties.loadTasksTopicPartitions).map { id ->
            ...
            ConsumerBatchWorker(
                consumer = consumer,
                eventHandler = object : ConsumerBatchEventHandler<LoadTask> {
                    override suspend fun handle(event: List<LoadTask>) {
    ...
    private fun createConsumerForLoadTasks(
        ...
        type: LoadType,
        id: Int
    ): RaribleKafkaConsumer<LoadTask> = RaribleKafkaConsumer(
        ...
        valueClass = LoadTask::class.java,
    ...
    private fun createProducerForLoadTasks(
        bootstrapServers: String,
        type: LoadType
    ): RaribleKafkaProducer<LoadTask> = RaribleKafkaProducer(
    ...
    private fun createProducerForNotifications(
        bootstrapServers: String,
        type: LoadType
